...
public class TemplateRenderingTests
{
    ...
    {
        var template =
            "Today is {{DATE}}{{#NAME}} and {{NAME}} is working{{/NAME}}{{#COMPANY}} at {{COMPANY}}{{/COMPANY}}{{#EMPTY}}to be removed{{/EMPTY}}";
    ...
    {
        var template =
            "Today is {{#DATE}}{{DATE}}{{#NAME}} and {{NAME}} is working{{/NAME}}{{/DATE}}{{#COMPANY}} at {{COMPANY}}{{#DATE}} on {{DATE}}{{/DATE}}{{/COMPANY}}{{#EMPTY}}to be removed{{/EMPTY}}";
    ...
    {
        var template =
            "The name is: {{#name}}{{name}}{{#else}}empty{{/name}}{{#company}} works at: {{company}}{{#else}} don't work{{/company}}";
        var dataOne = new { Name = "Guilherme", Company = "Guilherme Branco Stracini ME." };
    ...
    public void ValidateLoopTemplate()
    {
        ...
        var expected =
            $"<ul><li>{data.Li[0]}</li><li>{data.Li[1]}</li><li>{data.Li[2]}</li><li>{data.Li[3]}</li><li>{data.Li[4]}</li></ul>";
    ...
    /// <summary>
    /// Validates the rendering of a complex loop template using a specified data structure.
    /// </summary>
    /// <remarks>
    /// This test method checks if the template rendering engine correctly processes a template that includes a loop over a collection of person objects.
    /// The template is designed to create an unordered list (`<ul>`) where each person's first and last name is displayed as a list item (`<li>`).
    /// The method sets up a sample data structure containing multiple person entries and defines the expected output based on that data.
    /// It then invokes the template rendering service to generate the actual output and asserts that the generated output matches the expected result.
    /// This ensures that the template rendering logic is functioning as intended and can handle complex structures with loops.
    /// </remarks>
    [Fact]
    ...
    /// <summary>
    /// Validates the rendering of a template with provided data.
    /// </summary>
    /// <remarks>
    /// This test method constructs a template string that includes placeholders for a person's name and address details.
    /// It creates an anonymous object containing the name and address information, which includes the street, number, and city.
    /// The expected result is generated by interpolating the data into the template string.
    /// The method then uses a service locator to resolve an instance of <see cref="ITemplateRender"/> and calls its <c>Render</c> method,
    /// passing in the template and data. Finally, it asserts that the rendered result matches the expected output.
    /// This ensures that the template rendering logic is functioning correctly and that the placeholders are replaced with the correct values.
    /// </remarks>
    [Fact]
    public void ValidateWithTemplate()
    {
        var template =
            "{{NAME}} lives in {{#with ADDRESS}}{{STREET}}, {{NUMBER}} at {{CITY}}{{/with}}";
        var data = new
        {
            Name = "John Doe",
            Address = new
