...
namespace CrispyWaffle.Tests.Validations
{
    ...
    public class PersonalDataValidationTests
    {
        ...
        public void ValidateEmailAddress_ValidEmailAddress_ReturnsValidEmailAddress(string inputEmail, string expectedEmail)
        {
        ...
        {
            var exception = Assert.Throws<InvalidEmailAddressException>(inputEmail.ValidateEmailAddress);
            Assert.Equal($"The e-mail address '{inputEmail}' isn't in a valid e-mail address format", exception.Message);
        }
        ...
        {
            var exception = Assert.Throws<InvalidDocumentException>(() => inputDocument.IsValidBrazilianPersonDocument());
            Assert.Equal($"The value '{inputDocument}' isn't a valid value for a document of type CPF", exception.Message);
        }
        ...
        {
            var exception = Assert.Throws<InvalidDocumentException>(() => inputDocument.IsValidBrazilianCorporateDocument());
            Assert.Equal($"The value '{inputDocument}' isn't a valid value for a document of type CNPJ", exception.Message);
        }
    }
}
